						...
						for i in range(0, len(raw_bytes), 42):
							...
							if(raw_bytes[i] == 250 and raw_bytes[i+1] == ( 160 + i / 42)): #&& CRC check
								...
								for j in range(i+4, i+40, 6):
									...
									print(">r[",359-index,"]=", ranges / 1000.0, intensity );
...
def list_com_ports():
    ...
    for port in ports:
        # port.device: COM port name (e.g., COM3, /dev/ttyUSB0)
        # port.description: Human-readable description
...
def hexdump(data: bytes, width: int = 16) -> None:
    """
    Print a hex dump of the given bytes buffer.
    
    Each line shows:
    <offset in hex>  <width hex bytes>  <ASCII representation>
    """
    ...
    for offset in range(0, len(data), width):
        ...
        # Hex part: each byte as two hex digits
        hex_bytes = " ".join(f"{b:02X}" for b in chunk)
        
        # Pad hex part so ASCII aligns
        hex_padded = hex_bytes.ljust(width * 3 - 1)
        
        # ASCII part: printable chars or '.'
        ascii_part = "".join(chr(b) if chr(b) in string.printable and 32 <= b < 127 else "." for b in chunk)
        
        print(f"{offset:08X}  {hex_padded}  {ascii_part}")
...
if __name__ == "__main__":
    try:
        ...
        #    while True:
        #        # Read 64 bytes (blocks up to 1 second)
        #        data = ser.read(64)
        #        hexdump(data)
        poll()       
