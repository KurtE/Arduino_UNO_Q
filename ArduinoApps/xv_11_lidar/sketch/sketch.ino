...
const byte EEPROM_ID = 0x08;  // used to validate EEPROM initialized

double pwm_val = 200;          // start with ~50% power
double pwm_last;
double motor_rpm;
unsigned long now;
...
void setup() {
  ...
  analogWriteFrequency(xv_config.motor_pwm_pin, 5000);
  delay(100);
  ...
  if(!Bridge.provide("get_range", get_range)) {
    Serial.print("Error providing method: get_range\n"); delay(10);
  }
	delay(100);
}
...
void loop() {
  ...
  while (SerialX.available() > 0) {                  // read byte from LIDAR and relay to USB
    inByte = SerialX.read();                      // get incoming byte:
						...
						for (int ix = 0; ix < N_DATA_QUADS; ix++) {
							if (xv_config.aryAngles[startingAngle + ix]) {             // if we're supposed to display that angle
								if (aryInvalidDataFlag[ix] & BAD_DATA_MASK) {  // if LIDAR reported a data error...
									...
									//Serial.print(F(","));
									lidar.flag[startingAngle + ix] = 0;
									if (aryInvalidDataFlag[ix] & INVALID_DATA_FLAG){
										lidar.flag[startingAngle + ix] = 1;
										//Serial.println(F("I"));
									}
									if (aryInvalidDataFlag[ix] & STRENGTH_WARNING_FLAG){
										lidar.flag[startingAngle + ix] = 2;
										//Serial.println(F("S"));
									}
									lidar.angle[startingAngle + ix] = startingAngle + ix;
									//}
								}
								else {                                         // show clean data
									lidar.angle[startingAngle + ix] = startingAngle + ix;
									lidar.range[startingAngle + ix] = int(aryDist[ix]);
									lidar.quality[startingAngle + ix] = aryQuality[ix];
								}
							}
						}  // for (int ix = 0; ix < N_DATA_QUADS; ix++)
...
void initConfig() {
  ...
  xv_config.motor_pwm_pin = 3;

  xv_config.rpm_setpoint = 250;  // desired RPM
  xv_config.rpm_min = 100;
  xv_config.rpm_max = 300;
  xv_config.pwm_min = 100;
  xv_config.pwm_max = 1023;
  xv_config.sample_time = 20;
  xv_config.Kp = 2.0;  //old
  xv_config.Ki = 1.0;
  xv_config.Kd = 0.0;
...
void initSerialCommands() {
  ...
  sCmd.addCommand("GetRange", getRange);
}
...
void getRange(){
  //for(int r = 0; r < 360; r++) {
  //  Serial.printf("%d,%d,%d,%d\n", lidar.angle[r],lidar.range[r],lidar.quality[r], lidar.flag[r]); 
  //}
      for (int i = 0; i < 359; i++) {
        //Serial.print(": Angle=");
        Serial.print(lidar.angle[i]); Serial.print(", ");
        //Serial.print(", Flag=");
        Serial.print(lidar.flag[i]); Serial.print(", ");
        //Serial.print(", Quality=");
        Serial.print(lidar.quality[i]); Serial.print(", ");
        //Serial.print(", Range=");
        Serial.println(lidar.range[i]);
      }
      Serial.println("==================");
...
// Function returns a tuple of 4 vectors<int>
std::tuple<
    std::vector<int>, // angles
    std::vector<int>, // flags
    std::vector<int>, // qualities
...
> get_range() 
{
    ...
    std::vector<int> angles(lidar.angle, lidar.angle + 361);
    std::vector<int> flags(lidar.flag, lidar.flag + 361);
    std::vector<int> qualities(lidar.quality, lidar.quality + 361);
    std::vector<int> ranges(lidar.range, lidar.range + 361);

    // Return as tuple
    return std::make_tuple(angles, flags, qualities, ranges);
}
